...
from dmslstm.dmslstm_model import DMSLSTM
from dmslstm.edalstm_model import EDALSTM
...
# a function that returns the read_features dictionary
# based on the model topology
def get_read_features(topology):
    if topology == 'DMSLSTM':
        read_features = {
            'hourly': tf.io.VarLenFeature(dtype=tf.float32),
            'daily': tf.io.VarLenFeature(dtype=tf.float32),
    ...
    def _input_fn(params):
        ...
        dataset = dataset.map(
            lambda row: _parse_dataset_function(example_proto=row,
                                                read_features=dmslstm_features,
...
# ToDo: get topology from config_dict and use it to set tensor shapes accordingly
def set_make_input_fn_shapes(config_dict):
    if config_dict['topology'] == 'DMSLSTM':
        train_eval_objective_shapes = {
            'hourly': [config_dict['embedding']['hourly'], 1],
            'daily': [config_dict['embedding']['daily'], 1],
            'weekly': [config_dict['embedding']['weekly'], 1],
    ...
    elif config_dict['topology'] == 'EDALSTM':
        train_eval_objective_shapes = {
            'hourly': [config_dict['embedding']['hourly'], 1],
            'target': [config_dict['no_targets'], 1],
            'oh_wd': [7, 1],  # Monday to Sunday
...
def set_serving_input_fn_shapes(config_dict):
    if config_dict['topology'] == 'DMSLSTM':
        serving_objective_shapes = {
            ...
            'daily': [1, config_dict['embedding']['daily'], 1],
            'weekly': [1, config_dict['embedding']['weekly'], 1],
...
def time_series_forecaster(features, labels, mode, params):
    ...
    if params['precision'] == 'bfloat16':
        with tf.tpu.bfloat16_scope():
            # get topology from parameters
            if params['topology'] == 'DMSLSTM':
                forecaster_topology = DMSLSTM()
            elif params['topology'] == 'EDALSTM':
...
def main(unused_argv):
    ...
    # get the model topology identifier from the experiment id (DMSLSTM, EDALSTM, etc.)
    topology_string = experiment_id[:7]
    ...
    # also pass the topology string identifier
    configuration['topology'] = topology_string
