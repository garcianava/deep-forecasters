...
from forecasters.models import ARTRFDC
...
# there are five features on the ARTRFDC-SLDB, but only two of them
# are currently used for training and evaluation
# besides, ARTRFDC model do not use direct prediction, but iterative inference,
# therefore, just the two training features are parsed
...
def _parse_dataset_function(example_proto,
    ...
    # so:
    features_dict = {
        # from the first value to the one before last, include values and positional encodings
        'source': source
    }
    ...
    # that means [?, n_timesteps, 1] V.gr [?, 168, 1]

    # target positional encodings are required to be parsed for iterative inference
    # but not for training, evaluation, or direct prediction
    # so they can be parsed in the script/notebook for iterative inference
    ...
    if parse_timestamp:
        ...
        # source goes from the first value to the one before last
        features_dict['source_timestamp'] = source_timestamp
        # target goes from the second value to the last one
        features_dict['target_timestamp'] = target_timestamp
    ...
    def _input_fn(params):
        ...
        dataset = dataset.map(
            lambda row: _parse_dataset_function(example_proto=row,
                                                read_features=artrfdc_features,
...
def set_make_input_fn_shapes(config_dict):
    train_eval_objective_shapes = {
        'source': [config_dict['num_timesteps'], config_dict['model_dimension']],
        'target': [config_dict['num_timesteps'], 1]
    }
...
def set_serving_input_fn_shapes(config_dict):
    serving_objective_shapes = {
        'source': [1, config_dict['num_timesteps'], config_dict['model_dimension']],
        'target': [1, config_dict['num_timesteps'], 1]
    }
...
def time_series_forecaster(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
        ...
        # loss is calculated between forecasts, with shape (batch_size, n_timesteps, 1)
        # and target values, located at target/labels[:, :, :1], with shape (batch_size, n_timesteps, 1)
        # [:, :, :1] returns a tensor (?, n_timesteps, 1), while
        # [:, :, 0] returns a tensor (?, n_timesteps)
        # repeat this operation for additional metric_fn (RMSE)

        # ToDo: scheduled sampling
        #   here is the place where the current prediction value can be acquired
        #   during training!
        #   (print forecast to confirm)
    ...
    def serving_input_fn():
        ...
        features, _ = _parse_dataset_function(example_proto=feature_placeholders['example_bytes'],
                                              ...
                                              objective_shapes=set_serving_input_fn_shapes(
                                                  config_dict=config_dict),
                                              # do not parse timestamps during direct serving
                                              # parse them during iterative inference instead
                                              parse_timestamp=False)
        ...
        #   Also, considering the datetime timestamps will be useful later,
        #   it is better to change the input functions to add a feature dictionary
        #   for the autoregressive transformer.
        #   At the beginning, this feature_dict has the following keys:
        #   'source': [168, 7] - or n_timesteps, 1 value + 6 positional encodings
        #   'timestamps': string (to avoid system time differences in different computers)
